...
test("Gateway settings keeps existing status rendered during silent refresh", () => {
    const source = readFileSync(join(process.cwd(), "ui/src/components/settings/view/integrations/im/hooks/useGatewayStatus.ts"), "utf8");
...
test("Gateway settings starts weixin QR by begin route and ignores stale runtime errors", () => {
    const source = readFileSync(join(process.cwd(), "ui/src/components/settings/view/integrations/im/components/WeixinChannelSection.tsx"), "utf8");
