...
test("auto-compaction summarizes earlier work instead of deleting it", async () => {
    ...
    const messages = textMessages("Original task: update the workspace.", "Completed: wrote /tmp/project/output.json.", "Recent question: continue from the generated artifact.", "Assistant: inspecting the generated artifact.", "Recent status: artifact exists and is readable.", "Assistant: ready for the next instruction.", "Final recent question: continue.");
...
test("auto-compaction keeps the original transcript when summary generation fails", async () => {
    ...
    const messages = textMessages("Original task: preserve the artifact map.", "Completed: wrote /tmp/project/output.json.", "Recent question: continue.", "Assistant: waiting.", "Current status: artifact map is available.", "Assistant: ready.", "Final recent question: continue.");
    const runtime = new DefaultContextRuntime({
        tokenBudget,
        autoCompactionPolicy: new AutoCompactionPolicy({ tokenBudget }),
        compactionEngine: engine,
...
test("protected early tool turns do not bypass emergency compaction", async () => {
    ...
    const result = await runtime.tryAutoCompact({
        messages,
        budgetEvaluator: (candidate) => Promise.resolve(tokenBudget.snapshotFromTokens(textFrom(candidate).includes("[CONTEXT COMPACTION - REFERENCE ONLY]") ? 70 : 120, 100)),
    });
    assert.equal(result.type, "compacted");
...
test("80% pre-summary prune can stay below 90% without calling the summary model", async () => {
    ...
    const result = await runtime.tryAutoCompact({
        ...
        budgetEvaluator: (candidate) => Promise.resolve(tokenBudget.snapshotFromTokens(hasMicroMarker(candidate) ? 84 : 86, 100)),
    });
    assert.equal(result.type, "compacted");
...
test("summary runs before post-summary snip and keeps the compact checkpoint", async () => {
    ...
    const result = await runtime.tryAutoCompact({
        ...
        budgetEvaluator: (candidate) => Promise.resolve(tokenBudget.snapshotFromTokens(hasSnipBoundary(candidate) ? 75 : hasCompactSummary(candidate) ? 95 : 100, 100)),
    });
    assert.equal(result.type, "compacted");
