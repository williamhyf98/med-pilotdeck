...
function requestWith(message, toolCallId = "call-large-error") {
    return {
        model: "test-model",
...
test("large tool error references preserve error semantics for model replay", async () => {
    ...
    try {
        ...
        const anthropicTool = anthropic.messages[1]?.content.find((part) => part?.type === "tool_result");
        assert.equal(anthropicTool?.is_error, true);
        const google = buildGoogleRequest(requestWith(applied), model);
