...
import { AutoCompactionPolicy, buildPostCompactMessages, CompactionEngine, DefaultContextRuntime, MicroCompactionEngine, TokenBudgetManager, } from "../../src/context/index.js";
test("full compaction can disable protected turn preservation", async () => {
    ...
    assert.ok(summaryRequests[0].messages.some((message) => message.content.some((block) => block.type === "tool_call" && block.name === "Task")));
...
test("rolling checkpoints keep the accepted prefix byte-identical", async () => {
    ...
    const first = await engine.run({
        ...
        messages: Array.from({ length: 16 }, (_, index) => ({
            ...
            content: [{ type: "text", text: `Old work ${index} `.repeat(20) }],
        })),
        keepTailRatio: 0.2,
    });
    ...
    const second = await engine.run({
        ...
        messages: [
            ...
            ...Array.from({ length: 16 }, (_, index) => ({
                role: index % 2 === 0 ? "user" : "assistant",
                content: [{ type: "text", text: `New work ${index}` }],
            })),
        ],
        keepTailRatio: 0.05,
    });
...
test("auto full compaction keeps the best compacted result even when it still blocks", async () => {
    ...
    const result = await runtime.tryAutoCompact({
        messages: compactFixture(),
        budgetEvaluator: (candidate) => Promise.resolve(tokenBudget.snapshotFromTokens(hasToolCall(candidate, "Task") ? 130 : 120, 100)),
    });
    assert.equal(result.type, "compacted");
    assert.equal(summaryRequests.length, 2);
...
test("full compaction keeps the initiating user request with a protected tool cycle", async () => {
    ...
    assert.equal(compacted[requestIndex + 1]?.content.some((block) => block.type === "tool_call" && block.id === "protected-1"), true);
    assert.equal(compacted[requestIndex + 2]?.content.some((block) => block.type === "tool_result" && block.toolCallId === "protected-1"), true);
});
...
test("auto full compaction summarizes older tool groups inside one user task", async () => {
    ...
    const result = await runtime.tryAutoCompact({
        ...
        budgetEvaluator: (candidate) => Promise.resolve(tokenBudget.snapshotFromTokens(hasCompactSummary(candidate) ? 20 : 500, 100)),
    });
    assert.equal(result.type, "compacted");
...
test("summary input preserves thinking blocks from the summarized prefix", async () => {
    ...
    assert.equal(summaryRequests.length, 1);
    assert.ok(summaryRequests[0].messages.some((message) => message.content.some((block) => block.type === "thinking" && block.text.includes("private reasoning before the tail"))));
});
...
function summarizedThinkingFixture() {
    return [
        {
            role: "user",
...
function hasToolCall(messages, name) {
...
function hasToolResult(messages, toolCallId) {
    return messages.some((message) => message.content.some((block) => block.type === "tool_result" && block.toolCallId === toolCallId));
}
function hasToolResultReference(messages, toolCallId) {
    return messages.some((message) => message.content.some((block) => block.type === "tool_result_reference" && block.toolCallId === toolCallId));
}
...
function summaryPromptText(request) {
...
function hasCompactSummary(messages) {
    return messages.some((message) => message.content.some((block) => block.type === "text" && block.text.includes("[CONTEXT COMPACTION - REFERENCE ONLY]")));
}
function hasMicroCompactMarker(messages) {
    return messages.some((message) => message.content.some((block) => block.type === "tool_result" && toolResultText(block).includes("[Old tool result content compacted]")));
}
function tokenTailFixture() {
    const messages = [];
...
function singleUserToolChainFixture() {
    const messages = [
        {
            role: "user",
...
function fakeSnapshot(messages, tokenBudget) {
