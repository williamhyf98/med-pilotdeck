...
test("networkFetch times out requests", async () => {
    ...
    await assert.rejects(networkFetch("https://example.test", {}, { fetchImpl, timeoutMs: 1 }), { code: "network_timeout" });
});
test("networkFetch honors init.signal abort reasons without options.signal", async () => {
    ...
    await assert.rejects(networkFetch("https://example.test", { signal: controller.signal }, { fetchImpl }), { code: "network_timeout" });
});
test("networkFetch preserves parent NetworkFetchError reasons passed through options.signal", async () => {
    ...
    await assert.rejects(networkFetch("https://example.test", {}, { fetchImpl, signal: controller.signal }), { code: "network_timeout" });
});
