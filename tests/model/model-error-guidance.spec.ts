...
test("model request failure preserves provider raw message before action guidance", () => {
    ...
    const error = normalizeModelError("modelbest-openai", "openai", { error: { message: providerMessage, code: "model_not_found" } }, 404);
...
test("model_not_found guidance points users to local model settings", () => {
    const error = normalizeModelError("modelbest-openai", "openai", { error: { message: "The model `does-not-exist` does not exist" } }, 404);
...
test("stream idle timeout is classified as timeout with network and timeoutMs guidance", () => {
    const error = normalizeModelError("modelbest-openai", "openai", new Error("Stream idle timeout: no data received for 30000ms"));
...
test("billing and rate limit guidance distinguish provider-side fixes", () => {
    const billing = normalizeModelError("modelbest-openai", "openai", { error: { message: "insufficient balance, please top up your account" } }, 402);
    const rateLimit = normalizeModelError("modelbest-openai", "openai", { error: { message: "rate limit exceeded, retry later" } }, 429);
