...
test("expandMcpString expands ${pilotHome} placeholder", () => {
  const previous = process.env.PILOT_HOME;
  process.env.PILOT_HOME = "/tmp/pilotdeck-home-test";
  try {
    assert.equal(expandMcpString("${pilotHome}/plugins/med-tools/run.sh"), "/tmp/pilotdeck-home-test/plugins/med-tools/run.sh");
  } finally {
    if (previous === undefined) delete process.env.PILOT_HOME;
    else process.env.PILOT_HOME = previous;
  }
});
