...
test("read_file auto-pages large text files instead of failing", async () => {
    ...
    try {
        ...
        assert.equal(result.data.autoPaged, true);
        assert.ok(result.data.nextOffset > 1);
...
test("read_file explicit limit reads a large file range without auto paging", async () => {
    ...
    try {
        ...
        assert.equal(result.data.autoPaged, false);
        assert.equal(result.data.nextOffset, 3003);
    }
    finally {
        await rm(projectRoot, { recursive: true, force: true });
    }
...
test("read_file auto-shrinks oversized persisted tool-result ref ranges", async () => {
    ...
    try {
        ...
        assert.equal(result.data.autoPaged, true);
        assert.ok(result.data.endLine < 200);
        assert.ok(result.data.nextOffset > 1);
        assert.equal(result.metadata?.truncated, true);
    }
    finally {
...
test("read_file keeps explicit oversized ordinary file ranges strict", async () => {
    ...
    try {
        ...
        await assert.rejects(() => createReadFileTool().execute({ file_path: "large.txt", offset: 1, limit: 200 }, context(projectRoot)), /exceeds the text token budget/);
    }
    finally {
        await rm(projectRoot, { recursive: true, force: true });
    }
...
test("read_file range honors an aborted signal", async () => {
    ...
    try {
        ...
        controller.abort();
        await assert.rejects(() => readFileInRange(filePath, 1, 20, controller.signal), (error) => error instanceof Error
