...
function baseContext(fork, overrides = {}) {
    return {
        sessionId: "s1",
        turnId: "t1",
        cwd: process.cwd(),
...
test("agent tool accepts explorer as an alias for explore", async () => {
    ...
    const result = await tool.execute({ description: "inspect code", prompt: "inspect", subagent_type: "explorer" }, baseContext(createFork(calls)));
...
test("agent tool defaults general-purpose to explore in ask mode", async () => {
    const calls = [];
    const tool = createAgentTool();
    const result = await tool.execute({ description: "inspect code", prompt: "inspect" }, baseContext(createFork(calls), { runMode: "ask" }));
    assert.equal(result.data?.subagentType, "explore");
    assert.deepEqual(calls, ["explore"]);
});
test("agent tool preserves unknown custom fallback subagent names", async () => {
    ...
    const result = await tool.execute({ description: "custom run", prompt: "run", subagent_type: " CustomAgent " }, {
        sessionId: "s1",
        turnId: "t1",
        cwd: process.cwd(),
        permissionMode: "bypassPermissions",
