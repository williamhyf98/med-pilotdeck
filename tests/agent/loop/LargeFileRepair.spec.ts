...
function failure(toolName, message, options = {}) {
    return {
        type: "error",
        toolCallId: `call-${toolName}`,
        toolName,
...
test("ordinary edit failures after a successful write do not enter large-file repair", () => {
    ...
    assert.equal(repair.analyzeToolResults([success("write_file", { filePath: "/workspace/report.xlsx" })], NO_TRUNCATION), undefined);
    for (let attempt = 0; attempt < 6; attempt++) {
        assert.equal(repair.analyzeToolResults([failure("edit_file", "String to replace not found in file.")], NO_TRUNCATION), undefined);
    }
...
test("an explicit post-draft large-file failure starts bounded recovery", () => {
    ...
    for (let attempt = 1; attempt <= 5; attempt++) {
        const decision = repair.analyzeToolResults([failure("edit_file", "Tool output was truncated.", { code: "result_too_large" })], NO_TRUNCATION);
        assert.equal(decision?.type, "continue");
        assert.match(decision?.type === "continue" ? decision.prompt : "", new RegExp(`${attempt}/5`));
    }
    const stopped = repair.analyzeToolResults([failure("edit_file", "Tool output was truncated.", { code: "result_too_large" })], NO_TRUNCATION);
...
test("successful focused write clears an active large-file repair episode", () => {
    const repair = new LargeFileRepair();
    const started = repair.analyzeToolResults([failure("write_file", "The required parameter `content` is missing", {
...
test("permission failures are never reclassified as large-file recovery", () => {
    const repair = new LargeFileRepair();
    repair.analyzeToolResults([success("write_file", { filePath: "/workspace/report.xlsx" })], NO_TRUNCATION);
    assert.equal(repair.analyzeToolResults([failure("edit_file", "Permission denied", { code: "permission_denied" })], { ...NO_TRUNCATION, outputTruncated: true }), undefined);
    assert.equal(repair.analyzeToolResults([failure("edit_file", "Permission request cancelled", { code: "permission_cancelled" })], { ...NO_TRUNCATION, outputTruncated: true }), undefined);
});
