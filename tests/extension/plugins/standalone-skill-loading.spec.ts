...
async function writeSkill(skillDir, name, description, body) {
...
test("standalone skill precedence is project > user > builtin without legacy aliases", async () => {
    ...
    try {
        ...
        await writeSkill(join(builtinSkillsRoot, "docx"), "docx", "Built-in DOCX skill description.", "# Built-in DOCX skill");
        await writeSkill(join(pilotHome, "skills", "docx"), "docx", "Global DOCX skill description.", "# Global DOCX skill");
        await writeSkill(join(agentCwd, ".pilotdeck", "skills", "docx"), "docx", "Project DOCX skill description.", "# Project DOCX skill");
        ...
        await writeSkill(join(pluginDir, "skills", "docx"), "docx", "Plugin DOCX skill description.", "# Plugin DOCX skill");
        ...
        assert.equal(docxSkills.find((skill) => skill.name === "docx")?.path, join(agentCwd, ".pilotdeck", "skills", "docx", "SKILL.md"));
        assert.equal(docxSkills.find((skill) => skill.name === "office:docx")?.path, join(pluginDir, "skills", "docx", "SKILL.md"));
        ...
        await rm(join(agentCwd, ".pilotdeck", "skills", "docx"), { recursive: true, force: true });
        await runtime.refresh();
        assert.equal(runtime.getAllSkills().find((skill) => skill.name === "docx")?.description, "Global DOCX skill description.");
        assert.equal(runtime.getAllSkills().find((skill) => skill.name === "docx")?.path, join(pilotHome, "skills", "docx", "SKILL.md"));
        ...
        await runtime.refresh();
        assert.equal(runtime.getAllSkills().find((skill) => skill.name === "docx")?.description, "Built-in DOCX skill description.");
        assert.equal(runtime.getAllSkills().find((skill) => skill.name === "docx")?.path, join(builtinSkillsRoot, "docx", "SKILL.md"));
