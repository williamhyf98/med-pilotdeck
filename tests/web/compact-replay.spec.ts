...
function completedTurn(sessionId, turnId) {
    return {
        type: "success",
        sessionId,
        turnId,
...
test("web history restores structured records around a visible compact boundary", async () => {
    ...
    try {
        ...
        await storage.transcript.recordTurnResult(sessionKey, "turn-old", completedTurn(sessionKey, "turn-old"));
        ...
        await storage.transcript.recordTurnResult(sessionKey, "turn-compact", completedTurn(sessionKey, "turn-compact"));
        ...
        await storage.transcript.recordTurnResult(sessionKey, "turn-new", completedTurn(sessionKey, "turn-new"));
        ...
        const newUserIndex = replay.messages.findIndex((message) => message.kind === "text" && message.text?.includes("new user request after compact"));
...
test("subagent history keeps execution messages around its compact boundary", async () => {
    ...
    try {
        ...
        await sidechain.writer.recordTurnResult("subagent-session", "sub-turn-old", completedTurn("subagent-session", "sub-turn-old"));
        ...
        await sidechain.writer.recordTurnResult("subagent-session", "sub-turn-compact", completedTurn("subagent-session", "sub-turn-compact"));
        ...
        await sidechain.writer.recordTurnResult("subagent-session", "sub-turn-new", completedTurn("subagent-session", "sub-turn-new"));
        const replay = await readSubagentWebMessages({ sessionKey, subagentId }, { projectRoot, pilotHome });
        const text = replay.messages.map((message) => message.text ?? "").join("\n");
