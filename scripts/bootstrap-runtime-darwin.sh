#!/usr/bin/env bash
# Download Node + portable CPython into PilotDeck/.runtime for macOS (darwin),
# then install project deps. Mirrors scripts/bootstrap-runtime.sh (Linux) but
# uses darwin Node tarballs and apple-darwin python-build-standalone triples.
#
# Prefer: scripts/bootstrap-runtime-select.sh  (auto-picks linux vs darwin)
set -euo pipefail
...
os="$(uname -s)"
if [[ "$os" != "Darwin" ]]; then
  echo "error: bootstrap-runtime-darwin.sh is for macOS only (got ${os})." >&2
  echo "  use: ${SCRIPT_DIR}/bootstrap-runtime.sh          # Linux" >&2
  echo "  or:  ${SCRIPT_DIR}/bootstrap-runtime-select.sh   # auto-detect" >&2
  exit 1
fi
...
# Native npm modules (sharp, etc.) need a working compiler toolchain on macOS.
if ! xcrun --find clang >/dev/null 2>&1; then
  echo "error: Xcode Command Line Tools not found (needed for native Node deps)." >&2
...
echo "==> PilotDeck local runtime bootstrap (macOS / darwin)"
echo "    root:    ${PILOTDECK_ROOT}"
echo "    runtime: ${RUNTIME_DIR}"
echo "    arch:    ${arch} (node=${NODE_ARCH}, python=${PY_TRIPLE})"
echo "    tmp:     ${TMPDIR}"
...
install_node() {
  ...
  # Official macOS Node archives are .tar.gz (not .tar.xz).
  ...
  mv "${RUNTIME_DIR}/node-v${NODE_VERSION}-darwin-${NODE_ARCH}" "$RUNTIME_NODE_DIR"
...
install_python() {
  ...
  echo "==> Downloading portable CPython ${PY_VERSION} (${PY_TAG}, ${PY_TRIPLE})"
