...
分析本轮上传病例前，必须确认实际推理输入、病例标识、分割和 CT-CLIP 产物对应本轮附件。下面 smoke20 命令仅用于用户明确要求的环境验证/实验复现，不能用于回答上传病例。当前外部脚本若没有已验证的上传病例接入方式，应明确说明并退回 med-medical，不能拿示例数据的结果回答用户。
...
- 主答案模型与服务地址使用部署配置和外部脚本已有配置，不写死某台机器的地址。不要擅自切换模型。
...
1. 首次验证默认只做 dry-run，不调用答案模型，也不上传 CT：
   ...
   ```bash
   cd "${MED_DEEPCHEST_ROOT:?请先配置 DeepChest 工作区}" && \
...
3. 纯文本 Agent 运行会把 CT-CLIP、mask 和报告摘要组成文本 memory，使用部署配置的服务。以下命令仅用于明确要求的 smoke20 实验复现：
