---
name: med-medical
description: 解析通用医学附件（包括专用 CT 能力关闭时的完整 CT、PDF、报告截图、CDA/XML、检验文本、JSON、心电图/WFDB）。DICOM 必须先路由。
---

# 医疗多源解析（med-tools）

RADAR 和 DeepChest 由 `MED_SPECIALIZED_CT_ENABLED` 控制。关闭时不得调用
`med_radar_*` 或 `med_deepchest_*`；完整 CT 使用本 Skill 的通用医学解析。
此时直接在本 Skill 下调用 `med_dicom_route` 和 `med_parse_medical`，
无需加载独立的 DICOM 路由 Skill，也不要推荐隐藏的技能。
...
对 DICOM 文件或目录，先单独调用轻量的 `mcp__med-tools__med_dicom_route`，等待其完成后再规划；不得把路由和解析并行执行。若路由器推荐本 Skill（包括专用 CT 能力关闭时的完整 CT），继续使用 `med_parse_medical`。仅当路由器推荐专用流程且运行时允许时，才按相应 Skill 执行；工具返回 disabled 时不重试。
