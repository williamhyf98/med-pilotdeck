...
用户
  ...
  │                  └─ med_parse_medical → 纯解读直接保存 G9 报告；复合任务主模型继续交付
  │
...
| 信号 | 行动 |
|------|------|
| 上传 dcm/pdf/xml… + 解读/报告 | DICOM 先路由；**med-medical** 纯解读直接展示 `report`，复合任务用 material |
...
- 纯解读直接展示并保存 `report` 原文；复合任务的影像部分保留原文，主模型仅补充分析或后续交付。`care_plan` 仍按对应技能原样展示。
