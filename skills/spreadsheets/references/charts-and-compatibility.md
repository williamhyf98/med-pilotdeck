...
新建工作簿可用 sheet 级 `images[]` 插入浮动图片（ExcelJS `addImage`，锚在单元格附近），路径只用本地 `inbox` 绝对路径。图表继续走 `charts` + DrawingML，二者不要混用职责。

既有工作簿若已有绘图包，可能包含不支持的形状或图表关系：将任何绘图风险视为停止的理由，不要对既有文件做 `images` 插入。在已渲染页面中核验新图位置。
