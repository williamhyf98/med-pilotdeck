...
BUNDLED_FONT_DIR="${PPTX_SKILL_FONT_DIR:-$SKILL_DIR/../pdf/assets/fonts}"
...
cmd_check() {
  ...
  local node_ok=false deps_ok=false font_ok=false render_available=false
  ...
  printf '{"status":"%s","node":%s,"node_path":"%s","npm_path":"%s","dependencies":%s,"runtime":"%s","bundled_font":%s,"bundled_font_dir":"%s","libreoffice_path":"%s","pdf_renderer_path":"%s","render_available":%s,"legacy_ppt_conversion_available":%s}\n' \
...
runtime_missing_json() {
  printf '{"status":"error","code":"missing-dependencies","error":"交付包不完整：PPTX 本地运行时未就绪。请使用完整交付包，不要在现场安装依赖或自行编写 JavaScript。"}\n'
}
...
case "${1:-}" in
  ...
  bootstrap-runtime)
    ...
    ;;
  fix)
    printf '{"status":"error","code":"offline-install-disabled","error":"pptx.sh 不支持现场安装。请使用完整交付包，不要 npm，不要自行编写 JavaScript。"}\n' >&2
    exit 2
    ;;
  ""|-h|--help|help)
  ...
  *)
    ...
    export PPTX_SKILL_FONT_DIR="$BUNDLED_FONT_DIR"
