...
普通新建文档直接使用 `make`。只有修改已有文档的高级工作流才以
`capabilities`、`prepare --existing-document` 以及相关的
`schema` 开始。`prepare` 会冻结验收并返回当前
`PILOTDECK_WORK_DIR` 的规范路径；切勿猜测或搜索另一个轮次目录。
标准命令负责已声明操作；遇到 `unsupported` 或 `blocked` 时停止并说明
限制，不要编写 Python 回退。普通新建由 `make` 直接产生项目可见 DOCX；
修改已有文档时，每一次变更都会在 `PILOTDECK_WORK_DIR` 下生成内部候选
文件，并由 `deliver` 提升为项目可见的最终 DOCX。
...
1. 从可用上下文确认结果，不要编造事实。
2. 短文使用 `make --title --body --out`。
3. 对话里的长文先完整写入 Markdown，再使用
   `make --title --markdown --out`。
4. 复杂表格、图片、封面或精确结构使用严格 create spec，再交给
   `make --spec --out`；不要改写 Python 生成器。
5. `make` 内部执行 OOXML 校验和结构审计；LibreOffice 可用时还会生成
   页面预览。检查 `warning`，不能把缺失的视觉 QA 说成已完成。
6. 只有用户明确要求替换既有输出时才加 `--force`。
7. 最终只交付成功 JSON 中 `output` 指向的 DOCX；Markdown、spec、
   audit、候选和预览图不是交付物。

详细参数和 Markdown 映射见 [creation.md](creation.md)。
...
3. 更改一个段落样式；
4. 追加明确要求的章节；
5. 所需操作超出编辑模式时，返回 `unsupported` 或 `blocked` 并说明限制；
6. 不要调用 `fallback-patch` / `fallback-create`，不要手写 Python。
...
- 如果依赖缺失，报告交付包不完整并停止。不要运行 `fix` 或 pip。
...
- 如果文档依赖于签名、嵌入对象、自定义 XML 映射或复杂内容控件，保留源文件并报告不支持。
- 不要通过 `fallback-patch`、`fallback-create` 或包装器之外的脚本重试。
- 如果预检已通过但候选已更改，交付会被
