...
## 2026-09-21 全局维护设置补充（优先于下方旧兼容策略）

- PilotDeck 的已有与新建项目统一读取全局维护模式、时间间隔及推理模式；旧项目数据库设置保留但不再覆盖全局配置。独立使用 memory core 的调用方仍可使用项目设置。
- 设置页外层字段优先，旧 `memory.schedule` 仅逐字段兜底。旧全局配置只有间隔时仍解释为定时；用户在设置页切换后所有项目跟随，不擅自改写当前用户配置。
- 后台扫描、记忆面板手动维护和对话结束维护均按真实项目类型创建服务；战创伤使用专属分类提示词与仅 Feedback 创建硬闸。
- Dream 失败改为 1/2/4/8/16/32/60 分钟退避，不再将全局维护模式降级。锁忙仅跳过，不计模型失败；手动维护可跳过等待。配置热更新等待在途维护完成后释放其数据库。

## 2026-09-21 Task 11 范围修订（优先于下方原始设计）

用户已确认采用轻量提示词改造，不新增患者绑定、主体识别或患者/科研项目选择。

- 全局画像只保留身份背景、专业领域。回答偏好均属项目 Feedback，包括看起来全局有效的偏好；不恢复临床偏好/协作偏好画像段落。
- 项目是医学工作空间，不等于单一患者。病例、科研调查、教学、文献分析等有持续价值的内容都可提取为 project；沿用输入已有指代与来源，不额外管理患者身份。
- 提取保留时间、出处、适用范围、假设/疑似/否认/纠正等限定，不把助手推断变成确定事实，不把文献病例改写成当前患者事实。
- “这次/下一轮”等临时偏好交给会话上下文，通常不写长期 Feedback；“以后/默认/每次”等持续偏好才持久化，不扩大原适用范围。
- 通用医学新设置默认 immediate，回答结束后异步 Index，有内容变化才 Dream。保留现有锁和失败降级。
- 兼容策略：已保存的 immediate/interval/manual 原样保留；只有间隔而无模式的旧配置或旧数据库设置按 interval 解释，包含 0 的禁用语义。旧数据没有设置来源，不能可靠区分用户选择与历史默认值，因此不批量覆盖；已有项目可在记忆设置中手动选择 immediate。
- 本期不增加患者级隔离、不更改召回主干、不实现过敏史强制召回与 Dream 永不删除保障。原 11c 暂缓；原 11b 中全局格式偏好进入 user 的要求作废。

实施记录：通用医学分类和三类笔记提示词已按上述规则改写；复用共享 JSON、语言、覆盖测试片段；维护默认值、旧配置兼容和设置界面默认值同步调整。真实模型的语义遵循仍需实际会话验收，提示词快照测试不能证明分类准确率。

代表性语义验收用例（待配置模型实测，不视为自动测试已通过）：

| 输入 | 预期 |
|---|---|
| 我是急诊科主治，擅长多发伤 | user，身份背景与专业领域 |
| 我有糖尿病 | 不进入 user；有持续价值时 project |
| 患者青霉素过敏，疑似肺炎 | project，保留过敏史及“疑似” |
| 本研究排除青霉素过敏者 | project，纳排标准，不改写成患者过敏史 |
| 请记住这篇论文中病例的肾损伤结论 | project，保留论文来源和病例限定 |
| 假设患者过敏怎么办 | 不自动建立持久病史 |
| 下一轮用表格回答 | 不创建长期 Feedback |
| 以后病例汇报默认用表格 | feedback，保留病例汇报范围 |
| 我是骨科医生，这位患者过敏，以后先给结论 | 分别提取 user/project/feedback |

代码验证覆盖默认即时维护、无内容不执行、旧模式/间隔兼容、配置传递、设置路由、画像及隐私和作用域回归。原 11c 的患者级隔离和过敏史持久保护仍暂缓，Task 12 未实施。
