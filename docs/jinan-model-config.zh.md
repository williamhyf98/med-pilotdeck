...
> **⚠️ 本文是历史记录 + 示例，不是当前配置来源。** 下一轮优化 §2.6 已把
> `plugins/med-tools/plugin.json` 改成 `${env:NAME:-http://127.0.0.1:...}` 占位，
> 仓库里不再写死任何模型 IP。现场以 `config/deploy.env` 为唯一配置入口
> （从 [`config/deploy.env.example`](../config/deploy.env.example) 复制后填写），
> 下文出现的 `10.31.112.13` 仅表示济南那台机器当时的地址，**换机器时按实际填写**。
