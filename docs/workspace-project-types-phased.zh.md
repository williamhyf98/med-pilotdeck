...
/status: **P0–P7.1 已完成；下一步 P8**（按阶段推进；每阶段必须自测通过后再开下一阶段）
...
| **P7.1** | 工作区存储（看板「存储」）               | 查看并手动清理 `$WS` 与 archives；页内预览图/PDF/文本 | 中（UI，已完成） |
...
**不做（本阶段已交付部分）：** 完整「归档/工作区磁盘管理」产品页（见下方 **P7.1**）。本阶段仅磁盘归档 + 删除 API 返回归档路径即可。
...
/status: **已完成（2026-08-31）** — 交互骨架参考 med-integration「附件缓存」（能力与信息架构），主题使用 PilotDeck `neutral` + 蓝强调，未沿用其 Ant Design / 品牌色。

**入口与命名：**

- 主区看板溢出菜单（与技能 / 路由 / 记忆 / 常驻 / Cron 并列）增加一项 **「存储」**，走现有 `setActiveTab`，打开独立看板页（非聊天）。
- 页标题：**「工作区存储」**。

**范围（只扫、只删这两棵树）：**

- 在用：`$PILOT_HOME/workspaces/<typeKey>/<id>/`（`inbox` / `exports` / `scratch`）
- 归档：`$PILOT_HOME/archives/projects/<id>-<timestamp>/`
- **不含**对话 jsonl、项目记忆、插件目录、registry。本页不替代 P7 删项目。

**页面结构：**

- 页顶（两子 Tab 共用）：总占用、在用小计、归档小计（KB/MB/GB）；刷新；删除选中（已选数量 + 预计释放体积）；当前 Tab 内搜索（项目名 / 归档包名 / 文件名）。
- 页内两个子 Tab：
  1. **在用**：按项目折叠；组头为展示名、类型（通用医学 / 战创伤）、该项目 `$WS` 小计；组内再分 `inbox` / `exports` / `scratch`。文件行：勾选、文件名、相对路径、大小。组头可全选本组，或清空该项目工作区文件。
  2. **归档**：按归档包折叠；组头为原项目 id、归档时间、小计。可删单文件或整包。

**删除：**

- 粒度：单个文件，或整组（某个在用项目的工作区内容，或一整包归档）。
- 必须确认；文案写清：删的是磁盘文件、不可恢复、**不是删项目**。
- 在用项目清空后保留 `inbox` / `exports` / `scratch` 空目录骨架。
- 空的在用工作区不可在本页勾选或清空；从侧栏删除对应项目后，P7 会将其移出在用工作区。
- 归档组复选框代表整包归档；即使文件夹为空，也可勾选批量删除或直接删除整包目录。
- 路径必须落在上述两棵树内；失败列出未删项，禁止静默半删。删除成功后占用数字与列表实时刷新。

**预览（页内模态，不跳 Files）：**

| 类型 | 做法 |
| --- | --- |
| 图像（png / jpg / jpeg / gif / webp / bmp） | `<img>` |
| PDF | 浏览器内嵌 |
| 文本（txt / md / json / xml / csv 等） | 只读文本；大文件截断并提示 |
| 其它（docx、DICOM、ECG 等） | 不预览，标明类型，仍可删除 |

预览/下载只读接口同样只能读上述两棵树。不做 DICOM 帧渲染、共享引用告警。

**后端（实现时）：**

- `GET` 用量树：分组、每文件 path + sizeBytes、小计、合计、是否可预览。
- `GET` 预览/下载：文本设大小上限并截断。
- `POST` 删除：显式 kind（单文件 / 在用整组 / 归档整包）+ 相对安全路径。
- 第一期同步扫描即可，页上 loading；文件特别多再做成先目录汇总、点开再列文件。

**不做：** 跨机器配额、自动清理、回收站二次恢复、把本页做成删项目入口、改主题色。

**自测：**

- [x] 扫描 typed workspaces 与归档包，汇总文件 / 分组 / 总占用；符号链接不进入列表
- [x] 单文件删除与在用工作区整组清空；清空后重建 `inbox` / `exports` / `scratch`
- [x] 路径穿越在任何删除发生前被拒绝
- [x] 在用 / 归档子 Tab、文件选择与删除确认前端测试通过
- [x] 存储专项 7 个用例通过（含空归档整包删除与空在用工作区保护）；UI 生产构建通过
- [x] 全量 UI 测试已运行；仍有 14 个与本改动无关的既有失败（`node:sqlite` 测试环境、旧 MainArea 断言、streamSmoother、表格预览）
...
| P7   | **已完成**      | 2026-08-27 | 删项目：chats+记忆删除；`$WS` → `archives/projects/<id>-<ts>/`；确认文案；磁盘管理产品页见 P7.1 |
| P7.1 | **已完成**      | 2026-08-31 | 看板「存储」→ 工作区存储；在用/归档子 Tab；文件+整组删除；图/PDF/文本页内预览；见 §P7.1 |
| P8   | 待开始          |            |                                                                                  |
| P9   | 待开始          |            |                                                                                  |
