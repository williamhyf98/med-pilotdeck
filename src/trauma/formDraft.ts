...
function normalizePrimaryIntent(intent: TraumaInputIntent): TraumaPrimaryIntent {
  return intent === "domain_question_no_case" ? "knowledge_question" : intent;
}

function validateExtractedPreferenceSpans(
  rawText: string,
  ...
  return preferences.flatMap((preference) => {
    ...
    const directive = preference.directive.trim();
    if (!sourceSpan || !directive || !rawText.includes(sourceSpan)) return [];
    return [{ ...preference, sourceSpan, directive }];
  });
}
...
export function normalizeTraumaIntentPlan(
  ...
  const primaryIntent = normalizePrimaryIntent(extractedInputIntent(extracted));
  return {
    primaryIntent,
    scopeReason: extracted.scopeReason?.trim() ?? "",
