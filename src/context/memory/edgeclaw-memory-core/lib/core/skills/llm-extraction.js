...
import { GENERAL_MEDICINE_PROFILE, } from "./prompts/index.js";
...
function normalizeSectionMarkdown(value, headingPattern) {
 ...
 */
export function buildUserProfileBodyFromParsedSections(payload) {
    const identityContent = normalizeSectionMarkdown(payload.identity_background_markdown ?? payload.identity_background, /^#{1,6}\s*身份背景\s*\n+/i);
    const specialtyContent = normalizeSectionMarkdown(payload.specialty_markdown, /^#{1,6}\s*专业领域\s*\n+/i);
    const sections = [];
    if (identityContent)
        sections.push(`## 身份背景\n${identityContent}\n`);
    if (specialtyContent)
...
export class LlmMemoryExtractor {
    ...
    logger;
    prompts;
    constructor(config, runtime, logger, promptProfile) {
        this.config = config;
    ...
    async createMemoryNote(input) {
        ...
        if (!this.prompts.allowedTypes.includes(input.kind)) {
            this.logger?.info?.(`[clawxmemory] note kind "${input.kind}" is not allowed by profile "${this.prompts.type}" — discarding`);
            return null;
        }
