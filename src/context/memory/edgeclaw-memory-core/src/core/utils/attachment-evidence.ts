import type { MemoryAttachmentEvidence } from "../types.js";

/** Validate persisted/captured evidence with a budget independent of chat text. */
export function normalizeAttachmentEvidence(value: unknown): MemoryAttachmentEvidence[] {
  ...
  return value.slice(0, 64).flatMap((item) => {
    if (!item || typeof item.sourceId !== "string"
      ...
      || !Array.isArray(item.chunks)) return [];
    const chunks: string[] = [];
