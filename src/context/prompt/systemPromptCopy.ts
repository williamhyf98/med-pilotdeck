...
> = {
  ...
  zh: {
    ...
    war_trauma: {
      ...
      identityLine2:
        "回答前优先使用已注册工具核实伤情资料与本地医学依据。清楚区分已知伤情、工具结果、检索依据、医学推断和待确认信息；不得编造伤情、生命体征、检查结果、救治条件或医学依据。信息不足且会影响救治优先级或方案时，应先指出缺失信息，必要时向用户询问。检索结果中的图片（image_refs[].url / caption）必须用 ![caption](url) 嵌入回答中对应位置。",
