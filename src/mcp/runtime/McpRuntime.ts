  ...
  constructor(
    ...
    for (const spec of servers) {
      const clientOptions: McpClientOptions = {
        ...
        // Prefer per-server timeout from plugin/mcp.json; keep global default otherwise.
        callTimeoutMs:
          typeof spec.timeoutMs === "number" && Number.isFinite(spec.timeoutMs) && spec.timeoutMs > 0
            ? Math.floor(spec.timeoutMs)
            : this.options.clientOptions?.callTimeoutMs,
      };
      this.clients.set(spec.id, new McpClient(spec, clientOptions));
    }
  }
