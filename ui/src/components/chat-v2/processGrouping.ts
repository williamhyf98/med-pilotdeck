...
export function getToolTarget(message: ChatMessage): string {
  return (
    ...
    getToolInputString(message, 'glob_pattern') ||
...
function truncateToolText(text: string, max = 48): string {
...
function getToolStepTargetMeta(message: ChatMessage): { target: string; context: string } {
  ...
  const targetDirectory =
    getToolInputString(message, 'target_directory') ||
    getToolInputString(message, 'working_directory');
  const description =
    getToolInputString(message, 'description') ||
  ...
  if (filePath) {
    return {
      target: getDisplayTarget(filePath) || truncateToolText(filePath, 40),
      context: '',
    };
  }
  ...
  if (pattern) {
    const location = targetDirectory
      ? getDisplayTarget(targetDirectory) || truncateToolText(targetDirectory, 28)
      : '';
    return {
      target: truncateToolText(pattern, 40),
      context: location ? `in ${location}` : '',
    };
  }
  ...
  if (description) {
    return {
      target: truncateToolText(description, 48),
      context: '',
    };
  }
...
function getToolStepResultDetail(message: ChatMessage): string {
  ...
  const isSearchTool = /glob|grep|search|find|rg/.test(toolName);
  if (isSearchTool) {
    const foundMatch = content.match(/found\s+(\d+)\s+files?/i);
    if (foundMatch) {
      const count = Number(foundMatch[1]);
      return `Found ${count} ${count === 1 ? 'file' : 'files'}`;
    }
    return `Found ${lines.length} ${lines.length === 1 ? 'file' : 'files'}`;
  }
...
/**
 * Build a vertical timeline step list for process panels.
 * Includes tool name + short target (path / pattern / command) and optional result summary.
 */
export function buildProcessToolSteps(messages: ChatMessage[]): ProcessTraceStep[] {
  ...
  for (const message of messages) {
    ...
    const stepId = String(message.toolId || message.id || `${toolName}-${steps.length}`);
...
export function getLiveProcessToolNameSteps(group: LiveProcessGroup): ProcessTraceStep[] {
  return buildProcessToolSteps(group.messages);
}
