...
export function CitationChunkModal({
  ...
  return createPortal(
    <div
      role="dialog"
      aria-modal="true"
      aria-label={displayIndex > 0 ? `引用 ${displayIndex} 原文` : '未引用知识块原文'}
      ...
      <div className="flex max-h-[80vh] w-full max-w-2xl flex-col overflow-hidden rounded-lg bg-white shadow-2xl dark:bg-neutral-900">
        <div className="flex items-start gap-3 border-b border-neutral-200 px-5 py-4 dark:border-neutral-700">
          <span className="mt-0.5 shrink-0 rounded bg-blue-100 px-1.5 py-0.5 text-xs font-medium text-blue-700 dark:bg-blue-900 dark:text-blue-300">
            {displayIndex > 0 ? `[${displayIndex}]` : '未引用'}
          </span>
...
export function CitationPopover({
  ...
  // 流式候选元数据未到时，先保留上标样式；元数据到达后启用原文弹窗。
  if (!cite) {
    return <sup className="inline-flex items-center"><span className="rounded bg-blue-100 px-1 text-xs font-medium text-blue-700 dark:bg-blue-900 dark:text-blue-300">{children ?? (index ? `[${index}]` : null)}</span></sup>;
  }
  ...
  return (
    <>
      ...
      <Tooltip inline content={isOpen ? undefined : tooltipContent} position="top" delay={200}>
