...
export function createRemarkCitationPlugin(
  ...
  return function remarkCitation() {
    return (tree: MarkdownAstNode) => {
      transformCitationNodes(tree, citationMap, displayIndexMap, allowUnresolved);
    };
  };
}
...
function transformCitationNodes(
  ...
  for (let i = 0; i < node.children.length; i += 1) {
    ...
    if (child.type === 'text' && child.value) {
      const parts = splitByCitation(child.value, citationMap, allowUnresolved);
    ...
    transformCitationNodes(child, citationMap, displayIndexMap, allowUnresolved);
  }
}
...
function splitByCitation(
  ...
  while ((match = INLINE_CITATION_RE.exec(text)) !== null) {
    ...
    if (!citationMap.has(index) && !allowUnresolved) continue;
