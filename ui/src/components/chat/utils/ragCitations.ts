 ...
 *
 * 按正文首次出现顺序追加编号，流式期间既有编号不跳变。
 * 战创伤已持久化的 displayIndex 优先，保证右侧快照和历史回看一致。
 */
export function buildCitationDisplayMap(
  ...
  const assigned = new Map(citations.filter(c => c.displayIndex !== undefined).map(c => [c.index, c.displayIndex!]));
  // Include the shared turn map in the footer even when earlier citations occur in another bubble.
  for (const [index, number] of assigned) display.set(index, number);
  [...used].forEach((index, position) => display.set(index, assigned.get(index) ?? position + 1));
  return display;
}
...
export function mergeCitationsStable(
  ...
  const signature = merged
    .map((c) => `${c.index}:${c.displayIndex ?? ''}:${c.chunkId ?? ''}:${c.label ?? ''}:${c.text?.length ?? 0}`)
    .join('|');
