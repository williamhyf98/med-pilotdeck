...
function normalizeAttachmentRelativePath(value, fallbackName) {
        ...
        try {
            ...
            if (req.body?.relativePaths) {
                try {
                    ...
                    if (Array.isArray(parsed)) relativePaths = parsed.map((item) => String(item || ''));
                } catch {
                    return res.status(400).json({ error: 'Invalid relativePaths JSON' });
                }
            }
            if (relativePaths.length > 0 && relativePaths.length !== req.files.length) {
                return res.status(400).json({ error: 'relativePaths length must match attachments count' });
            }

            const useTree = relativePaths.length === req.files.length && relativePaths.some((item) => String(item).includes('/'));
            let pathOnlyIndexes = new Set();
            if (req.body?.pathOnlyIndexes) {
                try {
                    const parsed = JSON.parse(req.body.pathOnlyIndexes);
                    if (Array.isArray(parsed)) {
                        pathOnlyIndexes = new Set(parsed.map((item) => Number(item)).filter((item) => Number.isInteger(item) && item >= 0));
                    }
                } catch {
                    return res.status(400).json({ error: 'Invalid pathOnlyIndexes JSON' });
                }
            }
            const images = [];
            const files = [];
            ...
            for (const [index, file] of req.files.entries()) {
                ...
                let storedFile;
                if (useTree || (relativePath && relativePath.includes('/'))) {
                    storedFile = await moveUploadedAttachmentWithRelativePath(
                        file,
                        attachmentDir,
                ...
                // Folder uploads force path-only: never return multimodal data-URL images.
                if (CHAT_ATTACHMENT_IMAGE_MIMES.has(file.mimetype) && !useTree && !pathOnlyIndexes.has(index)) {
                    const buffer = await fsPromises.readFile(storedFile.path);
                    images.push({
            ...
            let folderPath = null;
            if (useTree && attachmentDir) {
                folderPath = attachmentDir;
            }
            ...
            res.json({
                ...
                files,
                folderPath,
                attachmentDir: files.length || images.length ? attachmentDir : null,
            });
        } catch (error) {
            ...
            const message = error instanceof Error ? error.message : 'Failed to process attachments';
