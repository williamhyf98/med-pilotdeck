 ...
 * User-scope overrides are rejected, including for administrators.
 */
...
export function resolveScopeTarget(actor, requestedScopeId) {
  if (requestedScopeId !== undefined && requestedScopeId !== null) {
    throw Object.assign(new Error('不支持切换到其他用户的数据作用域。'), {
      code: 'USER_SCOPE_OVERRIDE_DISABLED',
    });
  }
...
export async function withUserScope(req, res, next) {
  if (req.get?.(USER_SCOPE_HEADER) !== undefined || req.query?.scopeUser !== undefined) {
    res.status(403).json({ error: '不支持用户作用域代入。', code: 'USER_SCOPE_OVERRIDE_DISABLED' });
    return;
  }
  if (!isolationEnabled()) {
    next();
...
export async function buildUserScopeFor(user, requestedScopeId) {
  if (requestedScopeId !== undefined && requestedScopeId !== null) {
    resolveScopeTarget(user, requestedScopeId); // reject even in bypass mode
  }
